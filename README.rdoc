...
===:validate
Make sure that values fit some kind of criteria.  If you use a Proc to do validation, there are two ways to specify failure:  returning false or raising a HasEasy::ValidationError.  The former is demonstrated here.
  class User < ActiveRecord::Base
    has_easy :prefs do |p|
